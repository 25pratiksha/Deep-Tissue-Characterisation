function [net,stats] = cnn_train_dag_ConvAE(net, imdb, getBatch, varargin)
...
%                                                          Generate options
% -------------------------------------------------------------------------
opts = generate_default_opts_ConvAE();
...
for t=1:opts.batchSize:numel(subset)
  ...
  fprintf('%s:\t epoch %02d: %3d/%3d: %.1f Hz', mode, state.epoch, fix(t/opts.batchSize)+1, ceil(numel(subset)/opts.batchSize), stats.num/stats.time * max(numGpus, 1));
  fprintf('\t[obj-loss = %.3f%%]', stats.objective);
  fprintf('\n') ;
end
